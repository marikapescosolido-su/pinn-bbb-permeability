...
# Research question and note from the author
...
This work was produced as part of Equitech Futures' ["AI for Science Summer School"](https://www.futureshub.com/cohorts/ai-for-science-summer-school), guided by Dr. Mlinar and Dr. Mishra. The purpose of this document is to facilitate the understanding of PINNs set on a medical/clinical context, providing guidance on the concept of PINN itself and how Neural Networks (NNs) work. However, it does assume NN prior knowledge. Thus, if the reader thinks would benefit from a deeper understanding of NNs, I recommend this [online resource (Chapter 1)](http://neuralnetworksanddeeplearning.com/chap1.html).

# Context

Imagine that a patient is injected with a radioactive sugar tracer (such as FDG) and then enters a PET scanner, which takes repeated snapshots monitoring the radioactivity "flow" of the tracer in the brain. FDG is a modified glucose molecule that, despite undergoing the same phosphorylation process as natural glucose and turning into FDG-6 (when the molecule attaches to a phosphate group), it is not continuously broken down for energy. When FDG is in its FDG-6 form, due to its new chemical structure, it cannot be further metabolized and thus becomes trapped in neural cells.
...
This is known as the "trapped tissue phase" and is indicated with C2(t). Note that C2(t)k3 indicates the FDG that is being trapped in the brain cells (where k3 is the rate of trapping), whereas C2(t)k4 indicates the FDG that is leaving the brain cells and going back to the free tissue phase (where k4 is the rate of untrapping). However in real tissue k4 is very slow and approximated to 0.
...
The PET scanner cannot distinguish these three measurements, but rather sees them as combined radioactivity in each area over time as a unique signal: CPET(t) for "combined PET over time".
...
Among the four rates (\(K_1\), \(k_2\), \(k_3\), and \(k_4\)), the one describing BBB permeability is \(K_1\). The others describe the flux of FDG across the brain-tissue compartments, which is not of interest for the purpose of this work directly, but that are similarly predicted by the PINN. 

# Purpose

Let us recover the unknown K1 (and k2, k3, k4) from the known CPET(t) using a Physics-Informed Neural Network (PINN).
...
For the scope of this work, the "experimental" data are synthetically produced by adding noise to the physics equations that generate them.

## Physical parameters

Physics parameters existing only to generate "real" data. They are not known and rather represent what the PINN tries to discover. They are useful to generate the synthetic dataset. After the dataset-generation, we can assume that "they do not exist".
...
These values were taken from the following paper: "Tomographic measurement of local cerebral glucose metabolic rate in humans with (F-18)2-fluoro-2-deoxy-D-glucose: validation of method". Note once again that these values are unknown to the scientist when performing the PINN analysis.

The Cp(t) value is known to the scientist. The PET raw output is a 4D image, meaning a 3D space, changing over time, t. If you select a single voxel, you can see the time evolution of the radioactivity in that voxel, which is the CPET(t) value, the combined PET signal, mentioned previously. On the other hand, the Cp(t) is the plasma concentration of the FDG tracer in the blood (the injected sugar and input of the system). 

In a medical context the Cp(t) is obtained through periodic blood sampling, which is invasive and not always feasible. Or, through an image-derived input function (IDIF) from the PET images themselves, which is less invasive but more complex and less accurate. The latter would mean having in the PET scan also a large blood vessel (such as the aorta or a heart chamber) visible and selecting a voxel in them, treating its CPET(t) as the Cp(t) of the system. However, this is not always possible, and even when it is, the IDIF is not very accurate. 

Usually the first option (blood sampling) is adopted in combination with the Feng model. The Feng formula is a curve-fitting model that describes the time course of the plasma input function (Cp(t)) for FDG in humans. It is based on a sum of exponentials and is widely used in PET studies to model the plasma input function. Using the Feng model makes it possible to transform those scattered blood samples data points (or IDIF), into a smooth mathematical function that can be used in the ODEs describing the FDG motion across the brain tissue compartments. Since for this synthetic data generation we are not interested in the Cp(t) accuracy, we will use a simple function to generate it, which is not realistic but serves the purpose of this work. In fact, it could be possible to use examples of sample blood measurements and then apply the Feng model, but given the complexity of the 7-parameter Feng model, it is not valuable for the goal of this reading, which wants to focus on BBB permeability instead.
...
to calculate (C_1), we need to know how quickly (C_1) is changing, that is, (dC_1/dt). However, to know (dC_1/dt), we need to know (C_1): It is a vicious circle!
The only known value is (C_1(0)=0), because there has been no injection yet. This breaks the vicious circle, hence this is the starting point to solve the ODEs.

NB. (C_1(t)) is the accumulated result of the small changes (dC_1/dt) added together over time, starting from (t=0).

At (t=0), where the "circle breaks":
...
So, given the initial C1 and C2, I can calculate the rate. Known the rate, I can calculate the subsequent C1 and C2. Known the subsequent C1 and C2, I can calculate the new rate, ecc... First exact values, then calculation of the rate. This procedure should continue until t+ndt=t+max is reached, where the final cumulative C1(t_max) and C2(t_max) will be found.
...
#skipping the initial condition at 0, that is already correct because equal to zero.
#use len so that it indicates positions in the array and not values.
```


STEP 1: Use initial condition + calculate the rate
...
Here, we simulated a grid of t_syn of 60,001 points for accuracy. However, in a real PET scan there are only a limited number of useful snapshots, and here only 10 of them are presented.

```python
N_data = 10
t_data = np.linspace(t_min, t_max, N_data) 
#This generates a sequence of evenly spaced numbers over a specified interval, controlled 
#primarily by a start value, a stop value, and the total number of points.
```

```python
C1_data = np.interp(t_data, t_syn, C1_syn)
C2_data = np.interp(t_data, t_syn, C2_syn)
#Here np.interp wants to take for each of the 10 points in t_data, the two t_syn values that 
#are closestt to it (one just below, one just above) and interpolate its C1 or C2. 
#For example, if t_data is 6.5, you take the C1 at t_syn=6.499 and t_syn=6.501 and by 
#interpolating their C1 (in this example it is a perfect average because it is exactly 
#in the middle, but more generally, it should be a weighted average), you find the C1 at t_data.
```

Now, combine the C1_data and C2_data just calculated into the CPET_true(t), the "un-noisy" CPET(t).
...
Now, add noise to CPET_true(t) to find CPET_exp(t), the "noisy" CPET(t):
...
# - With seed: You can reproduce your exact metrics, training loss curves, and 
#   final model weights every single run.
```

```python
noise_level = 0.05 #this indicates how large the fake measurement is.
CPET_exp = CPET_true + noise_level * np.random.randn(N_data)
#np.random.randn(N_data) draws numbers from a normal distribution (bell curve). 
#Hence, np.random.randn(N_data) takes N_data=10 random numbers on the bell curve and
#then multiplies by the noise to then add it to the real CPET. The reason why the 
#bell curve values are multiplied by the noise comes from the fact that real measurement 
#errors (e.g. PET scanner) are often not given by single glitches, but rather by the sum
#of many tiny independent sources of error that pile up. According to the Central Limit Theorem,
#whenever you add up many small independent random effects, the result tends to form a bell curve,
#even if the individual effects were not bell shaped themselves.
#This makes the noise added more realistic.
```
...
Hence, we have now calculated both the experimental data CPET_exp(t) that will be used for the PINN, and the real exact one that will be used for comparison to see the accuracy of the physics equations in combination with the experimental data in the PINN.

Convert t_data and CPET_exp used to train the PINN into tensors, since PyTorch needs its own tensor version of the data to train:
...
Let us follow a similar procedure for Cp, since it gets called later inside physics_loss where autograd required everything in torch:
...
The code above builds a collocation tensor so that the PINN can verify for other 200 random points in the time interval chosen, if the ODEs are followed. In fact, for the previous 10 points selected, the PINN checks if their predicted PET signals match the "experimental" ones. However, when data are missing experimentally (we have only 10), the PINN needs to verify that between the measurements the ODEs are respected. It will be shown later why this is useful when calculating the physics loss.

# Define the model

This section defines the NN structure: Here are sequentially added 2 hidden layers with 32 neurons (nodes). This Python code defines and creates a Multi-Layer Perceptron (MLP).

```python
class PINN(nn.Module): #nn.Module is the basic parent class for all NN.
    def __init__(self, n_hidden=32):
        super(PINN, self).__init__() 
#This allows PyTorch to find, store, differentiate, and optimize all the NN weights.
        self.net = nn.Sequential( 
#nn sequential constructs a pipeline of layers so that the output of each operation becomes the input of the subsequent one.
```
...
then, the tanh activation function is applied to the output of each of the 32 neurons separately. `Tanh` converts each output to a number between -1 and 1, while introducing nonlinearity so that the network can represent curved concentration trajectories (not only straight lines).
...
- 32 × 32 = 1,024 weights. 
Note that each of the 32 neurons received the previous 32 values and the additional new 32 numbers generated by the second hidden layer.
...
This layer receives the 32 hidden values and produces two outputs: 
- (1) C1(t)_pred 
- (2) C2(t)_pred
...
```python
    def forward(self, t): 
#forward defines how the input data travels through the network. 
#Specifically, it passes the time values through every operation in self.net:
#t -> Linear -> Tanh -> Linear -> Tanh -> Linear -> out
        out = self.net(t)          
        #shape (batch, 2)
        C1 = out[:, 0:1]           
        #first column -> C1 (this specific notation is used to keep its two-dimensional shape)
        C2 = out[:, 1:2]          
        #second column -> C2
        return C1, C2
```

The output containing the 2D array for C1(t)_pred and C2(t)_pred with respect to time will look like the following: 
...
#both hidden layers contain 32 neurons.
```

The training of the weights will be shown in the follwing sections. At this point, PyTorch is generating all the initial weights and biases. However, the training will then change them appropriately.

Here, the overall structure is summarized: 
```text
   Input
  t (time)
  1 number
...
Now, an automatic differentiator is needed:

```python
def derivative(y, x):
```

This computes the derivative of y with respect to x using autograd from torch y, x must be tensors with requires_grad=True for x.

```python
    return torch.autograd.grad(
        y, x, 
    #y is the quantity being differentiated (in this case C1_pred, C2_pred)
    #x is the variable with respect to which y is differentiated (in this case, time t)
        grad_outputs=torch.ones_like(y), 
    #creation of a tensor of 1s with the same shape as y to return the derivative at every t.
        create_graph=True
    )[0]
```

This will allow to calculate easily the derivatives of C1_pred and C2_pred:
```text
derivative(C1_pred, t) -> dC1/dt 
derivative(C2_pred, t) -> dC2/dt
```
...
The function above returns the physical rates used in the differential equations.

# Define the loss components (PINN)

Let us remember that the PINN predicts (C_{1,\mathrm{pred}}(t)) and (C_{2,\mathrm{pred}}(t)) from the provided (C_{\mathrm{PET,exp}}(t)). The PINN is trained on three kinds of error:
...
Thus, through these three errors we measure the error produced not only by the 10 PET data points, but also by the data predicted in between.
...
#Below are shown the two equations, using the LEARNABLE rate guesses 
#instead of the real (unknown-to-the-model) k1_real, k2_real, k3_real, k4_real:
...
#Here, lr is the learning rate. This controls the step size taken during each update. 
#Here it is selected Adam, one of the most popular optimization algorithms in deep learning. 
#model.parameters(): This hands all the trainable weights and biases in the neural network over to 
#the optimizer so it "knows" what to adjust.
```
...
lambda_ode = 1.0
lambda_ic = 1.0
#These can be written manually for a baseline attempt, but eventually a softadapt/self-adaptive 
#PINN can be executed.
```
...
#num epochs: it is usually set a high epoch count and let a 
#technique called Early Stopping decide when to halt. 
#VALIDATION LOSS: As long as validation loss is decreasing, the model is learning. 
#When validation loss stops improving and begins rising while training loss keeps dropping, 
#the model is just memorizing the training data. Set a "patience" threshold (e.g., 10–20 epochs). 
#If validation loss doesn't improve for that many epochs, stop automatically and restore the best weights.
```
...
Then, combine losses with weights:

```python
    loss = lambda_data*l_data + lambda_ode*l_ode + lambda_ic*l_ic
```

Then, backpropagation, which is the central mathematical algorithm that allows a neural network to learn from its errors, follows:
...
This below is an example og how the training process looks like for one fo the 6000 epochs:

```text
Network predicts C1 and C2
           ↓
Losses measure the mistakes
           ↓
Backpropagation finds which weights caused the mistakes
           ↓
Optimizer adjusts the network weights and k1, k2, k3, k4
           ↓
Network makes improved predictions
```

Hence, the losses are outside the network: they judge its outputs, and provide the information needed to correct its weights and kinetic rates.

This is a two-step cycle:
- Forward Pass: Input data flows forward through the network layers to produce a prediction, and the loss function computes a single number saying how wrong that prediction was. 
- Backward Pass (Backpropagation): The error flows backward through the network. Using the calculus chain rule, PyTorch calculates the gradient—the direction and magnitude to tweak each weight to reduce the loss—starting from the output layer back to the input.


# Answer to the research question

Obtain the final positive kinetic parameters estimated by the PINN and, ultimately, plot the graph showing how CPET(t)_true, CPET(t)_exp, and CPET(t)_pred change over time.
...
plt.scatter(t_data, CPET_exp, color='red', label='CPET_exp (noisy)')
plt.plot(t_plot, CPET_true_plot, 'k--', label='CPET(t)_true (exact)')
plt.plot(t_plot, CPET_pred_plot, 'b', label='CPET(t)_pred (PINN)')
...
::: {.callout-note title="PINN output"}
```text
============================================================
FINAL KINETIC PARAMETERS ESTIMATED BY THE PINN
============================================================
K1 = 0.6104  (blood → free brain tissue)
k2 = 0.0782  (free brain tissue → blood)
k3 = 1.1690  (free → phosphorylated tissue)
k4 = 0.4260  (phosphorylated → free tissue)

============================================================
ANSWER TO THE RESEARCH QUESTION
============================================================
The PINN-estimated value of K1 is 0.6104. K1 represents the blood-to-brain tissue FDG influx rate and is the kinetic parameter used in this model to characterize FDG transport across the blood-brain barrier (BBB).

Therefore, the estimated BBB FDG permeability/transport parameter for this synthetic PET dataset is K1 = 0.6104.
```
:::

# Acknowledgments
I extend my sincere gratitude to Dr. Mlinar and Dr. Mishra for their guidance throughout the brainstorming and writing of this work. I am also deeply grateful to the Equitech Futures Institute for awarding me an EFI scholarship and granting me Futures Hub "Learner+" access, which enabled me to attend the "AI for Science Summer School" free of charge.
