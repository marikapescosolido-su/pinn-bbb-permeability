...
Among the four rates (\(K_1\), \(k_2\), \(k_3\), and \(k_4\)), the one describing BBB permeability is \(K_1\). The others describe the flux of FDG across the brain-tissue compartments, which is not of interest for the purpose of this work directly, but that are similarly optimized by the PINN. 
...
Physics parameters existing only to generate "real" data. They are not known and rather represent what the PINN tries to discover. They are useful to generate the synthetic dataset. After its generation, we can assume "they do not exist".
